...
assert.equal(approved.length,420);
...
const newBatch=require('../content/ibge-2026/batches/batch-010.json').questions;
assert.equal(newBatch.length,100);
assert(newBatch.every(item=>approved.some(published=>published.id===item.id)));
assert(newBatch.every(item=>item.whyWrong.length===5&&item.explanation.length>=120));
...
console.log('Substituição editorial e lote 010: 141 arquivadas, 100 novas adicionais, 420 publicáveis: OK');
