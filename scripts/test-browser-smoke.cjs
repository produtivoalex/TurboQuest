...
(async () => {
  ...
  try {
    ...
    await page.locator('#reviewCount').getByText('1 erro · 1 dúvida').waitFor();
    await page.locator('#reviewKind').selectOption('wrong');
    await page.locator('#reviewNow').click();
    assert.equal(await page.locator('#statement').textContent(), first.statement, 'erro acessível na revisão voluntária');
    await page.locator('#quit').click();
    await page.reload();
    await page.locator('.hero-cta').click();
    await page.locator('#resume').click();
    assert.equal(await page.locator('#statement').textContent(), first.statement, 'revisão voluntária pode ser retomada');
    await page.locator('#quit').click();
    await page.locator('#endPaused').click();
    await page.locator('#openProfile').click();
    assert.equal(await page.locator('.achievement:not(.locked)').count() >= 1, true, 'microconquista visível no perfil');
